...
function addNewRow() {
    if (currentRows < MAX_ROWS) {
        ...
        for(let i = 0; i < currentColumns; i++)newRow.insertCell();
    }
    currentRows++;
    updateButtonColor();
}
...
function addNewColumns() {
    if (currentColumns < MAX_COLUMNS) for (const row of table.rows)row.insertCell();
    currentColumns++;
    updateButtonColor();
}
function remNewColumns() {
    if (currentColumns > MIN_COLUMNS) for (const row of table.rows)row.deleteCell(-1);
    currentColumns--;
    updateButtonColor();
}
